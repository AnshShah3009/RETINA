...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (x < 3 || x >= i32(params.width) - 3 || y < 3 || y >= i32(params.height) - 3) {
        // Write zero score (pack two bf16 zeros per u32)
        let idx = u32(y) * params.width + u32(x);
        let u32_idx = idx / 2u;
        let shift = (idx % 2u) * 16u;
        let existing = output_data[u32_idx];
        let mask = ~(0xFFFFu << shift);
        output_data[u32_idx] = existing & mask;
        return;
    }
    ...
    let score_u32 = bitcast<u32>(score);
    let bf16_score = score_u32 >> 16u;
    let idx = u32(y) * params.width + u32(x);
    let u32_idx = idx / 2u;
    let shift = (idx % 2u) * 16u;
    let existing = output_data[u32_idx];
    let mask = ~(0xFFFFu << shift);
    output_data[u32_idx] = (existing & mask) | (bf16_score << shift);
}
