...
fn get_val(x: i32, y: i32) -> f32 {
    ...
    return dog_curr[u32(cl_y) * params.width + u32(cl_x)];
}
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var k = 0u; k < 4u; k++) {
        ...
        if (abs(val) > params.threshold) {
            ...
            if (is_max || is_min) {
                // Edge response check
                let dxx = get_val(x+1, y) + get_val(x-1, y) - 2.0*val;
                let dyy = get_val(x, y+1) + get_val(x, y-1) - 2.0*val;
                let dxy = (get_val(x+1, y+1) - get_val(x+1, y-1) - 
                           get_val(x-1, y+1) + get_val(x-1, y-1)) / 4.0;
