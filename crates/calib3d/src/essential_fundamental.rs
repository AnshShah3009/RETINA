...
pub fn find_essential_mat_ransac(
    ...
    let model = res
        .model
        .ok_or_else(|| cv_core::Error::AlgorithmError("RANSAC failed".into()))?;
...
fn enforce_essential_constraints(e: &Matrix3<f64>) -> Result<Matrix3<f64>> {
    ...
    let u = svd.u.ok_or_else(|| {
        cv_core::Error::AlgorithmError("SVD U missing in essential constraints".to_string())
    })?;
    let vt = svd.v_t.ok_or_else(|| {
        cv_core::Error::AlgorithmError("SVD V^T missing in essential constraints".to_string())
    })?;
