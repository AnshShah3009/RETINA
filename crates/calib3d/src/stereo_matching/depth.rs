...
use super::{DisparityMap, StereoParams};
use cv_core::{project_point, CameraIntrinsics, Error, PointCloudf64 as PointCloud};
...
pub fn project_point_to_image(
    ...
    project_point(point, &intrinsics, None, None).ok_or_else(|| {
        Error::InvalidInput("Point depth (z) must be non-zero to project to image".to_string())
    })
}
...
pub fn filter_pointcloud_by_depth(
    ...
    if !filtered_colors.is_empty() {
        filtered = filtered
            ...
            .map_err(|e| Error::RuntimeError(format!("Core error: {}", e)))?;
    }
    Ok(filtered)
}
...
mod tests {
    use super::*;
    use crate::stereo_matching::DisparityMap;
