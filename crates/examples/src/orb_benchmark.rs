...
fn main() {
    ...
    println!(
        "Starting ORB Benchmark (Features: {}, Levels: {})",
        n_features, n_levels
    );
...
[GPU Benchmark]"
    ...
    let tensor_cpu: Tensor<u8, cv_core::storage::CpuStorage<u8>> =
        Tensor::from_vec(img.to_vec(), shape).unwrap();
