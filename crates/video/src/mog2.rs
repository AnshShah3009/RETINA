        ...
        match ctx {
            ComputeDevice::Gpu(gpu) => {
                ...
                let frame_gpu = frame.to_gpu_ctx(gpu).map_err(|e| {
                    Error::RuntimeError(format!(
                        "Invalid parameters: Upload to GPU failed: {:?}",
                        e
                    ))
                })?;
                let frame_f32 =
                ...
                let model_gpu = self
                    .model
                    .as_mut()
                    .ok_or_else(|| Error::RuntimeError("Model missing".to_string()))?
                    .downcast_mut::<Tensor<f32, GpuStorage<f32>>>()
                    .ok_or_else(|| {
                        Error::RuntimeError("Downcast to GPU model failed".to_string())
                    })?;
                ...
                let u8_data: Vec<u8> = mask_cpu
                    .as_slice()
                    .map_err(|_| Error::RuntimeError("Failed to get mask slice".to_string()))?
                    .iter()
                    .map(|&v| v as u8)
                    .collect();
            ...
            ComputeDevice::Cpu(cpu) => {
                ...
                let frame_f32_vec: Vec<f32> = frame_cpu
                    .as_slice()
                    .map_err(|_| Error::RuntimeError("Failed to get frame slice".to_string()))?
                    .iter()
                ...
                let model_cpu = self
                    ...
                    .ok_or_else(|| {
                        Error::RuntimeError("Downcast to CPU model failed".to_string())
                    })?;
...
mod tests {
    ...
    fn test_mog2_frame_dimension_change() {
        ...
        let width1 = 32usize;
        let height1 = 32usize;
        let frame1: CpuTensor<u8> = CpuTensor::from_vec(
            vec![128u8; width1 * height1],
            TensorShape::new(1, height1, width1),
        )
        .unwrap();
        ...
        let width2 = 48usize;
        let height2 = 48usize;
        let frame2: CpuTensor<u8> = CpuTensor::from_vec(
            vec![128u8; width2 * height2],
            TensorShape::new(1, height2, width2),
        )
        .unwrap();
